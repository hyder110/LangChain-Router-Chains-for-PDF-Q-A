...
def main():
    ...
    openai_api_key = None  # Initialize the variable here
    if 'OPENAI_API_KEY' not in os.environ:
        openai_api_key = st.sidebar.text_input('Enter your OpenAI API Key', type='password')
        if openai_api_key:
            os.environ['OPENAI_API_KEY'] = openai_api_key
    else:
        openai_api_key = os.environ.get("OPENAI_API_KEY")  # Set the variable value if API Key is in the environment
    ...
    chain = MultiRetrievalQAChain.from_retrievers(OpenAI(openai_api_key=openai_api_key), st.session_state.retriever_names, st.session_state.retriever_descriptions, st.session_state.retrievers, verbose=True)
